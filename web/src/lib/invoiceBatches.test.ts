...
import { describeRule, invoiceBatches } from "./invoiceBatches.ts";
...
test("every 4 projects: batches counted oldest first, newest shown first, last one partial", () => {
  ...
  assert.deepEqual(b.map((x) => x.label), ["Batch 2 (2/4)", "Batch 1"]);
...
test("month end: calendar months, the 31st stays in its own month", () => {
  ...
  assert.deepEqual(b.map((x) => [x.label, x.ids.join()]), [
    ["Sep 2026", "c"],
    ["Aug 2026", "a,b"],
    ["Jul 2026", "d"],
  ]);
...
test("no rule, no batches", () => {
  ...
  assert.equal(describeRule({ cadence: null, dayOfMonth: null, every: null }), null);
  assert.equal(describeRule({ cadence: "milestone", dayOfMonth: null, every: 4 }), "Invoiced every 4 projects");
  assert.equal(describeRule({ cadence: "monthly_date", dayOfMonth: 30, every: null }), "Invoiced at each month end");
});
